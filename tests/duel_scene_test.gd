...
func _init() -> void:
	...
	root.add_child(duel)
	await process_frame
	duel.enemy.visible = true
	duel.get_node("WorldContext").router.change_map("boss_arena")
	await process_frame
	...
	duel._choose_route(true)
	duel.player.position = Vector2(350.0, 430.0)
	duel.enemy.position = Vector2(400.0, 430.0)
	duel.enemy.set_intent("strike")
	duel.enemy.set_state("commit")
	duel.enemy.motion_facing = -1.0
	duel.phase = "telegraph"
	duel.timer = 0.01
	await process_frame
	duel._process(0.02)
	assert(duel.enemy_attack_resolved)
